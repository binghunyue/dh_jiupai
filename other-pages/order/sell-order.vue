				...
				<scroll-view scroll-y style="height: 100%;" @scrolltolower="onreachBottom">
					<view class="card" v-for="(item, index) in dataList" :key="index">
						...
						<view class="operation u-flex">
							...
							<u-button shape="circle" :custom-style="redBtnStyle" v-else-if="item.status == '待收款'" @click="goPay">待收款</u-button>
							<u-button shape="circle" :custom-style="redBtnStyle" v-else-if="item.status == '待确认收款'">待确认收款</u-button>
